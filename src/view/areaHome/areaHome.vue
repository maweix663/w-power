  ...
  <div class="containerHome">
    <div class="header">
      ...
      <div  class="rightBox">
        ...
        <!--<div @click="changeColor" class="changeColor"><img src="../../../static/img/changecolor.png"></div>-->
        <div>{{ userInfoId }}</div>
      </div>
    </div>
        ...
        <div class="spareParts"  :class="[skinColor?'moduleBgBlue':'moduleBgYellow']">
          ...
          <div class="moduleHome">
            <table class="myTable" :class="[skinColor?'tableBlue':'tableYellow']" id="nav_tab" >
              ...
              <tr style="height: 30px;">
                <!--<th :colspan="headTitle1.length">在岗人员情况</th>-->
                <th v-show="headTitle1.length > 0" :colspan="headTitle1.length" :style="tableBorderRight">在岗人员情况</th>
                <th v-show="headTitle2.length > 0" :colspan="headTitle2.length">防疫物资情况</th>
              </tr>
              <tr>
                <th v-for="(item1, index1) in headTitle1" :style="headTitle1.length == index1 + 1?tableBorderRight:''">{{ item1 }}</th>
              ...
              <tr>
                <td v-for="(item,index) in spareArr" :style="headTitle1.length-1 == index?tableBorderRight:''">{{ item }}</td></tr>
              </tbody>
            </table>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      changeColor: function () {
        this.skinCount = 2;//parseInt(this.skinCount)+1;
        if (this.skinCount % 2 == 0) {
          this.skinColor = true;
          this.conBg = "../../../static/img/backgroung-blue.png";
          this.mapStyle = 'amap://styles/normal';
        } else {
      ...
      isColor: function () {
        if (this.skinColor) {
          ...
          this.colorCommon.color2 = '#ffffff';
      ...
      getMaterialType() {
        ...
        this.http.post('/homePage/openSparePart', params)
          .then(res => {
            let data = res.data;
            if(data.people != null) {
              for(let i = 0; i < data.people.length; i++) {
                this.headTitle1.push(data.people[i].id);
                this.spareArr.push(data.people[i].number);
              }
            } else {
              this.headTitle1 = [];
            }
            if(data.spare != null) {
              for (let j = 0; j < data.spare.length; j++) {
                if(!(data.spare[j].materialId == 'N95口罩（不带气阀）' ||data.spare[j].materialId == '一次性头套')){
                  this.headTitle2.push(data.spare[j].materialId);
                  this.spareArr.push(data.spare[j].materialNumber);
                }
              }
            } else {
              this.headTitle2 = [];
