<template>
  <div class="containerAreaHomeMove">
  <div class="containerAreaHomeMoveMain" id="containerAreaHomeMove">
    <!--标题-->
    <div class="header">
    ...
    <div class="module">
      ...
      <div class="materialMod">
        <div class="rateBody">
          <ul>
            <li class="rateHead">
              <span v-for="(item, index) in headTitle1">{{item}}</span>
              <span v-for="(item, index) in headTitle2">{{item}}</span>
            </li>
            <li @click="ponitClick(item, index)">
              <span v-for="(item,index) in spareArr">{{ item }}</span>
            </li>
          </ul>
        </div>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      // 监听滚动
      watchScroll () {
        var read = document.querySelector('#containerAreaHomeMove').scrollTop
        if (read > 160) {
          ...
          .then(res => {
            ...
            if(data.people != null) {
              for(let i = 0; i < data.people.length; i++) {
                this.headTitle1.push(data.people[i].id + '（人）');
